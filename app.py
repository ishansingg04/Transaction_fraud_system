...
def get_dashboard_stats():
    ...
    dict_cursor.execute('''
        ...
        LIMIT 10
    ''')
    recent_txns = dict_cursor.fetchall()
    ...
    dict_cursor.execute('''
        ...
        LIMIT 5
    ''')
    active_alerts = dict_cursor.fetchall()
...
def get_transactions():
    ...
    # Explicitly convert datetime objects to string before returning via jsonify
    for t in transactions:
        if isinstance(t['date_time'], datetime):
            t['date_time'] = t['date_time'].strftime('%Y-%m-%d %H:%M:%S')
...
def create_transaction():
    ...
    try:
        ...
        cursor.execute("SELECT account_balance, risk_level FROM account_details WHERE account_number = %s", (account_number,))
        ...
        # Insert location
        cursor.execute("INSERT INTO location (location_id, ip_address, city_country, is_known_proxy) VALUES (%s, %s, %s, %s)",
        ...
        if result['final_status'] in ['COMPLETED', 'FLAGGED']:
             ...
             cursor.execute("UPDATE account_details SET account_balance = %s WHERE account_number = %s", (new_balance, account_number))
             conn.commit()
...
def resolve_alert(alert_id):
    ...
    try:
        ...
        # Update alert status
        cursor.execute("UPDATE fraud_alert SET alert_status = 'CLOSED' WHERE alert_id = %s", (alert_id,))
        
        if resolution == 'false_positive':
            # Unblock account and transaction
            cursor.execute("""UPDATE `transaction` SET status_code = 'COMPLETED' WHERE transaction_id = %s""", (alert['transaction_id'],))
            cursor.execute("UPDATE account_details SET risk_level = 'LOW' WHERE account_number = %s", (alert['account_number'],))
        elif resolution == 'confirm_fraud':
            # Leave as is, maybe ensure it's blocked
            cursor.execute("""UPDATE `transaction` SET status_code = 'BLOCKED' WHERE transaction_id = %s""", (alert['transaction_id'],))
            cursor.execute("UPDATE account_details SET risk_level = 'HIGH' WHERE account_number = %s", (alert['account_number'],))
            
            if blacklist_location and alert['ip_address']:
                blacklist_id = "BL" + str(uuid.uuid4()).split('-')[0].upper()
                cursor.execute("INSERT IGNORE INTO blacklist (blacklist_id, blocked_value, reason_code, date_added) VALUES (%s, %s, %s, %s)",
...
def get_blacklist():
    ...
    for b in blacklist:
        if isinstance(b['date_added'], datetime):
            b['date_added'] = b['date_added'].strftime('%Y-%m-%d')
        elif hasattr(b['date_added'], 'strftime'):
            b['date_added'] = b['date_added'].strftime('%Y-%m-%d')
...
def add_blacklist():
    ...
    try:
        ...
        cursor.execute("INSERT INTO blacklist (blacklist_id, blocked_value, reason_code, date_added) VALUES (%s, %s, %s, %s)",
...
def delete_blacklist(blacklist_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM blacklist WHERE blacklist_id = %s", (blacklist_id,))
        conn.commit()
